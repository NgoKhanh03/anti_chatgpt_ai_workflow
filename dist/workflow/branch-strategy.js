function slug(value) {
    const result = value
        ...
        .replace(/^-+|-+$/g, '');
    if (!result)
        throw new Error(`Cannot derive branch slug from ${value}`);
    return result;
}
export function resolveBranchStrategy(runId, phaseIds, config, mainBranch = 'main') {
    if (phaseIds.length === 0)
        throw new Error('Branch strategy requires at least one phase.');
    ...
    if (config.mode === 'custom') {
        for (const phaseId of customGates) {
            if (!phaseIds.includes(phaseId))
                throw new Error(`Unknown custom gate phase ${phaseId}`);
        }
    }
...
export function assertMergeAllowed(strategy, targetBranch, approvals) {
    if (targetBranch === strategy.finalMergeTarget) {
        if (strategy.mode === 'phase') {
            if (!approvals.phaseApproved)
                throw new Error('Phase approval is required before merging to main.');
        }
    ...
    if (targetBranch === strategy.integrationBranch) {
        if (strategy.mode === 'phase')
            throw new Error('Phase mode does not use an integration branch.');
        return;
    }
