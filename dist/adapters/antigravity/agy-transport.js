...
function parseChecks(value) {
    ...
    const source = value;
    const result = {};
    for (const key of ['lint', 'typecheck', 'tests', 'build']) {
        ...
        if (!CHECK_STATUSES.has(normalized)) {
...
function parseResponseJson(response) {
    if (!response?.trim())
        throw new Error('Antigravity response is empty.');
    const candidates = [];
...
export class AgyAntigravityTransport {
    ...
    failedResult(request, evidence, error) {
        return {
    ...
    async execute(request) {
        ...
        let envelope;
        try {
            envelope = JSON.parse(commandResult.stdout);
        }
        catch (error) {
            return this.failedResult(request, evidence, new Error(`Antigravity CLI returned invalid JSON: ${String(error)}`));
        }
        if (envelope.status !== 'SUCCESS') {
            return this.failedResult(request, evidence, new Error(`Antigravity CLI failed with status ${envelope.status ?? 'UNKNOWN'}: ${envelope.error ?? envelope.response ?? commandResult.stderr}`));
        }
        let reported;
        try {
            reported = parseStructuredResult(envelope.structured_output ?? parseResponseJson(envelope.response));
        }
        catch (error) {
            return this.failedResult(request, evidence, error);
        }
        if (reported.taskId !== request.task.task.id) {
            return this.failedResult(request, evidence, new Error(`Antigravity returned task ${reported.taskId}, expected ${request.task.task.id}.`));
        }
        return {
            ...reported,
