...
function validateFeature(feature, issues) {
    ...
    for (const criterionId of criterionIds) {
        if (!covered.has(criterionId))
            issues.push(`Feature ${feature.id} criterion ${criterionId} is not covered by any unit`);
    }
    try {
...
function validatePhase(phase, issues) {
    ...
    for (const criterionId of phaseCriterionIds) {
        if (!covered.has(criterionId))
            issues.push(`Phase ${phase.id} criterion ${criterionId} is not covered by any feature`);
    }
}
export function validatePlan(plan) {
    ...
    if (!Number.isInteger(plan.revision) || plan.revision <= 0)
        issues.push('Plan revision must be a positive integer');
    if (plan.phases.length === 0)
        issues.push('Plan requires at least one phase');
    ...
    for (const duplicate of duplicates(allFeatureIds))
        issues.push(`Duplicate feature id ${duplicate}`);
    const allUnitIds = plan.phases.flatMap((phase) => phase.features.flatMap((feature) => feature.units.map((unit) => unit.id)));
    for (const duplicate of duplicates(allUnitIds))
        issues.push(`Duplicate unit id ${duplicate}`);
    for (const phase of plan.phases)
        validatePhase(phase, issues);
