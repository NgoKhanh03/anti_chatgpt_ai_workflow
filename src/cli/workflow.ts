...
function parseOptions(argv: string[]): CliOptions {
  ...
  if (!taskFile) throw new Error('--task <task.json> is required.');
  return {
    command,
    taskFile,
...
export function parseTaskContract(value: unknown): TaskContract {
  if (!value || typeof value !== 'object') throw new Error('Task contract must be an object.');
  const source = value as Record<string, unknown>;
  if (!source.task || typeof source.task !== 'object') throw new Error('task must be an object.');
  ...
  return {
    ...
    scope: source.scope as TaskContract['scope'],
  };
}

function slug(value: string): string {
  const result = value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
  if (!result) throw new Error(`Cannot derive slug from ${value}.`);
  return result;
}

async function existingRelativeFile(repo: string, explicit: string | undefined, fallback: string) {
  const candidate = explicit ?? fallback;
  try {
    await access(resolve(repo, candidate));
    return candidate;
  } catch {
    if (explicit) throw new Error(`Context file does not exist: ${resolve(repo, explicit)}`);
    return undefined;
  }
}

export async function runWorkflowCli(argv = process.argv.slice(2)): Promise<void> {
  ...
  const lease = new RunLease(
    resolve(options.repo, `.ai/runs/${runSlug}/lock`),
    task.task.id,
  ...
  try {
    ...
    const antigravity = new AntigravityAdapter(new AgyAntigravityTransport(runner, {
      ...
      effort: (process.env.ANTIGRAVITY_EFFORT as 'low' | 'medium' | 'high' | undefined) ?? 'high',
